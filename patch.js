...
const names = [
    'fetchDashboard', 'fetchAnalytics', 'fetchProducts', 'fetchCategories', 
    'fetchOrders', 'fetchSettings', 'saveProduct', 'deleteProduct', 
    'saveCategory', 'deleteCategory', 'updateOrderStatus', 'updateInventory', 'saveSettings'
];

for (const name of names) {
    const rx = new RegExp(\`export async function \\b\${name}\\b\\\\([^\\\\)]*\\\\)\\\\s*\\{\`);
    code = code.replace(rx, (m) => m + '\\n    await requireAdmin();');
}
